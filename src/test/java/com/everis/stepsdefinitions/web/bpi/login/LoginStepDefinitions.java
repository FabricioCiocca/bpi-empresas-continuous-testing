...
import com.everis.stepsdefinitions.web.bpi.pruebas.AlertaAdmision;
...
/*        theActorCalled(empresa).attemptsTo(AbrirBpi.loginBpiPage());
    ...
    public void que_el_usuario_usuario06_accede_a_la_aplicacion_bpi_dev_si(String user, String ambiente, String credencialesOK) throws InterruptedException, FindFailed {
        theActorCalled(user).attemptsTo(AbrirBpi.loginBpiPage());
       /* if (ambiente.equals("DEV")) {
                theActorInTheSpotlight().attemptsTo(new AlertaAdmision());
        }*/

        if (ambiente.trim().equals("DEV")) {
            Screen s = new Screen();
            s.click("C://Users//fcioccah//Proyectos//bpi-empresas-continuous-testing//src//test//resources//Aceptar.png");
        }
        String password = "";
